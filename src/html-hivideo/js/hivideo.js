...
(function (global, factory) {
	...
	hivideo.prototype = {
		...
		init: function (ele) {
          ...
          <button hivideoId="fullBtn" class="onBtn"></button> \
			...
			this.video = ele;
			this.videoBar = hivideo.querySelector(
				'div[hivideoId="videoBar"]',
				videoWrapper
			);
			this.videoPlayIcon = videoCenterPlayer;
			this.playBtn = hivideo.querySelector(
				'button[hivideoId="playBtn"]',
				videoWrapper
			);
			this.fullBtn = hivideo.querySelector(
				'button[hivideoId="fullBtn"]',
				videoWrapper
			);
			this.muteBtn = hivideo.querySelector(
				'button[hivideoId="muteBtn"]',
				videoWrapper
			);
			this.seekbar = hivideo.querySelector(
				'input[hivideoId="seekbar"]',
				videoWrapper
			);
			this.curTimeSpan = hivideo.querySelector(
				".video-time-current",
				videoWrapper
			);
			this.totalTimeSpan = hivideo.querySelector(
				".video-time-total",
				videoWrapper
			);
			this.volumebar = hivideo.querySelector(
				'input[hivideoId="volumebar"]',
				videoWrapper
			);
		...
		config: function () {
			...
			[
				{ autoplay: boolConvert },
				{ isrotate: boolConvert },
				{ autoHide: boolConvert },
	...
	hivideo.prototype.bindFullEvent = function () {
		...
		[
			...
			document.addEventListener(eventType, function (event) {
				if (
					(curFullhivideoId = document.body.getAttribute("curfullHivideo")) &&
					curFullhivideoId !== self.videoId_
				) {
					return;
				}
				...
				if (self.isFullScreen()) {
					...
					if (isRotate)
						self.videoParent.className = self.videoParent.className
							.replace("rotate90", "")
							.trim();
		...
		self.fullBtn &&
			self.fullBtn.addEventListener("click", function () {
				if (!self.isFullScreen()) {
	...
	hivideo.prototype.bindAutoHideControlsEvent = function () {
		...
		var showOrHideControls = function (isShow) {
			isShow
				? hivideo.removeClass(self.videoBar, "none")
				: hivideo.addClass(self.videoBar, "none");
		};
	...
	hivideo.prototype.bindTimeChangeEvent = function () {
		...
		//拖动时间进度
		self.seekbar &&
			self.seekbar.addEventListener("change", function () {
		...
		self.video &&
			self.video.addEventListener("timeupdate", function () {
				...
				self.curTimeSpan.innerText = hivideo.getTimeBySecond(
		...
		self.video &&
			self.video.addEventListener("loadedmetadata", function () {
				self.totalTimeSpan.innerText = hivideo.getTimeBySecond(
					self.video.duration
				);
			});
		//播放结束
		...
		self.volumebar &&
			self.volumebar.addEventListener("change", function () {
